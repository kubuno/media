...
use crate::services::{ffprobe, scanner};
        ...
        if is_video {
            ...
            if !ep_exists {
                ...
                let show_id: Uuid = sqlx::query_scalar!(
                    r#"INSERT INTO media.tv_shows (library_id, name, meta_status)
                       VALUES ($1, $2, 'pending_meta')
            ...
            let (artist_name, album_name, title) = scanner::parse_audio_path(entry.path());
            let info = ffprobe::probe(&settings.transcoding.ffprobe_bin, &file_path)
                .await
                .unwrap_or_default();
            ...
            // artist_id/album_id backfilled if they were previously NULL.
            sqlx::query!(
...
pub async fn index_single_file(
    ...
    if lib_type == "shows" {
        ...
        // Audio
        let (artist_name, album_name, title) =
